...
@section('content')
    <div class="subheader flex-wrap">
        <div class="subheader-left pt-2 pb-2">
            <h3 class="subheader-left-title">{{__('Transfers')}}</h3>
        ...
        <div class="card mb-4">
            <div class="card-header">
                <div class="card-title">
                    <h3 class="card-label">{{__('Transfers')}}</h3>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-end justify-content-between flex-wrap">
                    <form action="{{route('transfers.index')}}" class="ml-auto mb-4">
                ...
                <div class="table-responsive">
                    <table class="table mobile-table">
                        ...
                        <tr>
                            ...
                            <th>@sortablelink('address', __('Address'))</th>
                            <th>@sortablelink('price_per_km', __('Price Per KM'))</th>
                            <th>@sortablelink('car_size', __('Car Size'))</th>
                        ...
                        <tr>
                            ...
                            <td>{{$transfer->price_per_km}}</td>
                            <td>{{$transfer->car_size}}</td>
                            <td>{{$transfer->created_at}}</td>
                            <td class="table-buttons">
                                <div class="text-center table-buttons">
                                    <a href="{{route('transfers.show', $transfer->id)}}" class="btn" title="{{__('Show details')}}">
                                        <i class="flaticon-eye"></i>
                                    </a>
                                    <a href="{{route('transfers.edit', $transfer->id)}}" class="btn" title="{{__('Edit details')}}">
                                        <i class="flaticon-edit"></i>
                                    </a>
                                    <a href="#" class="btn" title="{{__('Delete')}}" onclick="$(this).next().submit()">
                                        <i class="flaticon2-trash"></i>
                                    </a>
                                    <form action="{{route('transfers.destroy', $transfer->id)}}" method="POST" onsubmit="return confirm('{{__('Are You Sure?')}}')">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
