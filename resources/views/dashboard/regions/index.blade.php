...
@section('content')
    <div class="subheader flex-wrap">
        <div class="subheader-left pt-2 pb-2">
            <h3 class="subheader-left-title">{{__('Regions')}}</h3>
    ...
    <div class="container-fluid">
        <div class="card mb-4">
            <div class="card-header">
                ...
                <div class="ml-auto">
                    <a href="{{route('regions.create')}}" class="btn btn-create">
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-end justify-content-between flex-wrap">
                    <form action="{{route('regions.index')}}" class="ml-auto mb-4">
                        ...
                        <tr>
                            ...
                            <td class="table-buttons">
                                <div class="text-center table-buttons">
                                    <a href="{{route('regions.show', $region->id)}}" class="btn" title="{{__('Show details')}}">
                                        <i class="flaticon-eye"></i>
                                    </a>
                                    <a href="{{route('regions.edit', $region->id)}}" class="btn" title="{{__('Edit details')}}">
                                        <i class="flaticon-edit"></i>
                                    </a>
                                    <a href="#" class="btn" title="{{__('Delete')}}" onclick="$(this).next().submit()">
                                        <i class="flaticon2-trash"></i>
                                    </a>
                                    <form action="{{route('regions.destroy', $region->id)}}" method="POST" onsubmit="return confirm('{{__('Are You Sure?')}}')">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
